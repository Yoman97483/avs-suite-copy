alter table public.interventions
  add column if not exists duplicated_from_intervention_id uuid
    references public.interventions(id) on delete set null;

create index if not exists interventions_duplicated_from_idx
  on public.interventions(duplicated_from_intervention_id)
  where duplicated_from_intervention_id is not null;

create unique index if not exists interventions_duplicate_once_idx
  on public.interventions(duplicated_from_intervention_id, employee_id, date)
  where duplicated_from_intervention_id is not null;

create table if not exists public.intervention_duplication_skips (
  id uuid primary key default gen_random_uuid(),
  source_intervention_id uuid not null references public.interventions(id) on delete cascade,
  employee_id uuid not null references public.employees(id) on delete cascade,
  target_date date not null,
  created_at timestamp with time zone not null default now(),
  unique (source_intervention_id, employee_id, target_date)
);

alter table public.intervention_duplication_skips enable row level security;

drop policy if exists intervention_duplication_skips_admin_all
  on public.intervention_duplication_skips;

create policy intervention_duplication_skips_admin_all
  on public.intervention_duplication_skips
  for all
  to authenticated
...
grant select, insert, delete on public.intervention_duplication_skips to authenticated;
revoke all on public.intervention_duplication_skips from anon;
...
with_flags as (
  select
    ...
    case
      ...
      else greatest(0, (extract(epoch from (c.actual_end_calc - c.actual_start_calc)) / 60)::integer) <= (c.planned_min - 5)
    end as pb_time,
    case
